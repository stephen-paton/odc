...
IdentifierToken :: struct {
    value: string,
}

IdentifierToken__try_from_str :: proc(value: string, allocator := context.allocator, loc := #caller_location) -> (token: IdentifierToken, err: Token__Err__try_from_str) {
    err = ._Ok
